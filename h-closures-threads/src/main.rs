...
fn main() {
    ...
    //
    let sum = handle.join().expect("Couldn't join summing thread.");
    ...
    // the child threads.
    let (tx_c, rx_c): (crossbeam::Sender<&str>, crossbeam::Receiver<&str>) = channel::unbounded();
    let rx_d = rx_c.clone();


    let handle_c = thread::spawn(move || {
        for received in rx_c {
            println!("Thread C: {}", received);
        }
    });


    let handle_d = thread::spawn(move || {
        for received in rx_d {
            println!("Thread D: {}", received);
        }
    });
